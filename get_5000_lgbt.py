...
output_file = "5000_lgbt_english_sample.csv"

hd = gd.hyperstream_directory(update_file=gd.HYPERSTREAM_OUTNAME)
cd = gd.hyperstream_directory(directory=gd.GZIP_PATH, update_file=gd.USA_OUTNAME, gzips=True)

# just lgbt stuff
cd = cd[cd['topic'] == "lgbtq"]
...
def r(n, collected=None, m=2, exclude=None):
    ...
    unchecked = gd.random_sample(cd, collect_int * m, disclude_tweet_ids=disclude)
